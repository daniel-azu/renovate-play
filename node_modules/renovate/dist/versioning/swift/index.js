...
const getSatisfyingVersion = (versions, range) => maxSatisfying(versions.map((v) => v.replace(/^v/, '')), (0, range_1.toSemverRange)(range));
const minSatisfyingVersion = (versions, range) => minSatisfying(versions.map((v) => v.replace(/^v/, '')), (0, range_1.toSemverRange)(range));
const isLessThanRange = (version, range) => ltr(version, (0, range_1.toSemverRange)(range));
const matches = (version, range) => satisfies(version, (0, range_1.toSemverRange)(range));
exports.api = {
    equals,
    getMajor,
    getMinor,
