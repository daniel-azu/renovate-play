...
async function initPlatform({ endpoint, token, username, gitAuthor, }) {
    ...
    if (username) {
        ...
        userDetails = await (0, user_1.getUserDetails)(defaults.endpoint, token);
        renovateUsername = userDetails.username;
    }
    let discoveredGitAuthor;
    if (!gitAuthor) {
        userDetails = await (0, user_1.getUserDetails)(defaults.endpoint, token);
        const userEmail = await (0, user_1.getUserEmail)(defaults.endpoint, token);
        if (userEmail) {
...
async function getRawFile(fileName, repo = config.repository) {
...
async function getJsonFile(fileName, repo = config.repository) {
...
async function initRepo({ endpoint, repository, forkMode, forkToken, renovateUsername, cloneSubmodules, ignorePrAuthor, }) {
    ...
    config = { repository, cloneSubmodules, ignorePrAuthor };
    // istanbul ignore if
    if (endpoint) {
    ...
    let repo;
    try {
        repo = await githubApi.queryRepo(graphql_1.repoInfoQuery, {
            variables: {
        ...
        try {
            ...
            if (forkDefaultBranch !== config.defaultBranch) {
                ...
                try {
                    ...
                    if (((_c = (_b = err.response) === null || _b === void 0 ? void 0 : _b.body) === null || _c === void 0 ? void 0 : _c.message) === 'Reference already exists') {
                        logger_1.logger.debug(`Branch ${config.defaultBranch} already exists in the fork`);
                    }
                    else {
                        logger_1.logger.warn({ err, body: (_d = err.response) === null || _d === void 0 ? void 0 : _d.body }, 'Could not create parent defaultBranch in fork');
                    }
                }
    ...
    if (forkMode) {
        ...
        const tokenType = ((_e = opts.token) === null || _e === void 0 ? void 0 : _e.startsWith('x-access-token:'))
            ? 'app'
            : 'personal access';
