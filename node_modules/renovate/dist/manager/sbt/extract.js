...
const isPackageFileVersion = (str) => /^(version\s*:=\s*).*$/.test(str);
...
const isVarSeqSingleLine = (str) => /^\s*(private\s*)?(lazy\s*)?val\s+[_a-zA-Z][_a-zA-Z0-9]*\s*=\s*Seq\(.*\).*\s*$/.test(str);
const isVarSeqMultipleLine = (str) => /^\s*(private\s*)?(lazy\s*)?val\s+[_a-zA-Z][_a-zA-Z0-9]*\s*=\s*Seq\(.*[^)]*.*$/.test(str);
