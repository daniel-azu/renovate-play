...
function getSourceCredentialVars(pyprojectContent, packageFileName) {
    ...
    for (const source of poetrySources) {
        const matchingHostRule = (0, host_rules_1.find)({ url: source.url });
    ...
    try {
        ...
        if (isLockFileMaintenance) {
            ...
            for (let i = 0; i < updatedDeps.length; i += 1) {
                ...
                cmd.push(`poetry update --lock --no-interaction ${(0, shlex_1.quote)(dep.depName)}`);
            }
        }
        const tagConstraint = getPythonConstraint(existingLockFileContent, config);
        const poetryRequirement = ((_a = config.constraints) === null || _a === void 0 ? void 0 : _a.poetry) || 'poetry';
        const poetryInstall = 'pip install ' + poetryRequirement.split(' ').map(shlex_1.quote).join(' ');
