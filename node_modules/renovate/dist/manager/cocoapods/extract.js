...
function gitDep(parsedLine) {
    ...
    const platformMatch = /[@/](?<platform>github|gitlab)\.com[:/](?<account>[^/]+)\/(?<repo>[^/]+)/.exec(git);
    if (platformMatch) {
        const { account, repo, platform } = (platformMatch === null || platformMatch === void 0 ? void 0 : platformMatch.groups) || {};
        if (account && repo) {
