...
function generateBranchUpgradeCache(upgrade) {
    const { datasource, depName, lookupName, fixedVersion, currentVersion, newVersion, currentDigest, newDigest, sourceUrl, } = upgrade;
    return {
        datasource,
        depName,
        lookupName,
...
async function generateBranchCache(branch) {
    ...
    try {
        ...
        if (((_a = err.response) === null || _a === void 0 ? void 0 : _a.statusCode) === 404) {
            logger_1.logger.warn({ err, branchName }, '404 error when generating branch cache');
            return null;
        }
...
async function setBranchCache(branches) {
    ...
    (0, repository_1.getCache)().branches = branchCache.filter(Boolean);
}
