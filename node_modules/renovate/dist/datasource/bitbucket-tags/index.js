...
class BitBucketTagsDatasource extends datasource_1.Datasource {
    constructor() {
        ...
        this.bitbucketHttp = new bitbucket_1.BitbucketHttp();
    }
    static getRegistryURL(registryUrl) {
        // fallback to default API endpoint if custom not provided
        return registryUrl !== null && registryUrl !== void 0 ? registryUrl : this.defaultRegistryUrls[0];
    }
    static getCacheKey(registryUrl, repo, type) {
        return `${BitBucketTagsDatasource.getRegistryURL(registryUrl)}:${repo}:${type}`;
    }
    // getReleases fetches list of tags for the repository
    async getReleases({ registryUrl, lookupName: repo, }) {
        const url = `/2.0/repositories/${repo}/refs/tags`;
        const bitbucketTags = (await this.bitbucketHttp.getJson(url)).body;
        const dependency = {
            sourceUrl: `${(0, url_1.ensureTrailingSlash)(BitBucketTagsDatasource.getRegistryURL(registryUrl))}${repo}`,
    ...
    async getDigest({ lookupName: repo, registryUrl }, newValue) {
...
__decorate([
    (0, decorator_1.cache)({
        ...
        key: ({ registryUrl, lookupName }) => BitBucketTagsDatasource.getCacheKey(registryUrl, lookupName, 'tags'),
    })
...
__decorate([
    (0, decorator_1.cache)({
        namespace: BitBucketTagsDatasource.cacheNamespace,
        key: ({ registryUrl, lookupName }) => BitBucketTagsDatasource.getCacheKey(registryUrl, lookupName, 'digest'),
    })
