...
(async () => {
    ...
    for (const file of app_strings_1.configFileNames.filter((name) => name !== 'package.json')) {
        try {
            const rawContent = (0, fs_extra_1.readFileSync)(file, 'utf8');
    ...
    try {
        const pkgJson = JSON.parse((0, fs_extra_1.readFileSync)('package.json', 'utf8'));
        if (pkgJson.renovate) {
    ...
    try {
        const fileConfig = (0, file_1.getConfig)(process.env);
