...
async function fetchPreset({ pkgName, filePreset, presetPath, endpoint, fetch, }) {
    ...
    if (fileName === 'default') {
        try {
            jsonContent = await fetch(pkgName, buildFilePath('default.json'), endpoint);
        }
        catch (err) {
            ...
            jsonContent = await fetch(pkgName, buildFilePath('renovate.json'), endpoint);
        }
    }
    else {
        jsonContent = await fetch(pkgName, buildFilePath(`${fileName}.json`), endpoint);
    }
    if (!jsonContent) {
